...
exports.start = function(config)
{
  ...
  function done_create_beer(req, res) {
    var doc = normalize_beer_fields(req.body);
    var brewery_id = doc.brewery_id.replace(' ', '_').toLowerCase();
    var beer_id = brewery_id + '-' + doc.name.replace(' ', '-').toLowerCase();
  ...
  // of GET method, it renders a form. And in case of POST it updates the
  // document in couchbase and redirects the client.
  function begin_edit_brewery(req, res) {
    db.get(req.params.brewery_id, function(err, result) {
      var doc = result.value;
      if( doc === undefined ) { // Trying to edit non-existing doc ?
        res.send(404);
        ...
        doc.id = req.params.beer_id;
        var view = { is_create: false, brewery: doc };
        res.render('brewery/edit', view);
      }
    });
  }
  function done_edit_brewery(req, res) {
    var doc = normalize_brewery_fields(req.body);

    db.get( req.params.brewery_id, function(err, result) {
      console.log(doc.brewery_id);
      if (result.value === undefined) { // Trying to edit non-existing doc ?
        res.send(404);
      } else {    // Set and redirect.
        db.set( req.params.brewery_id, doc, function(err, doc, meta) {
          res.redirect('/breweries/show/'+req.params.brewery_id);
        })
      }
    });
  }
  app.get('/breweries/edit/:brewery_id', begin_edit_brewery);
  app.post('/breweries/edit/:brewery_id', done_edit_brewery);

  // Create a new beer document. Same as edit, only that we use add() API
  // instead of set() API.
  function begin_create_brewery(req, res) {
  ...
  function done_create_brewery(req, res) {
    var doc = normalize_brewery_fields(req.body);
    var brewery_id = doc.id;
    db.add( brewery_id, doc, function(err, result) {
      if (err) throw err;
      res.redirect('/breweries/show/'+brewery_id);
    });
  }
  app.get('/breweries/create', begin_create_brewery);
  app.post('/breweries/create', done_create_brewery);
...
function normalize_beer_fields(data) {
  ...
  if (!doc['brewery_id']) {
    ...
    doc['brewery_id'] = doc['brewery_name'].replace(' ', '-');
  }
...
// utility function to validate form submissions - creating / editing beer
// documents.
function normalize_brewery_fields(data) {
  var doc = {};
  _.each(data, function(value, key) {
    if(key.substr(0,7) == 'brewery') {
      doc[key.substr(8)] = value;
    }
  });

  if (!doc['name']) {
    throw new Error('Must have brewery name');
  }
  if (!doc['id']) {
